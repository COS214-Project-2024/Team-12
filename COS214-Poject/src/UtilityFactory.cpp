...
// UtilityFactory.cpp - simplified version
std::shared_ptr<UtilityFlyweight> UtilityFactory::getUtility(const std::string& type) {
    // Return the existing flyweight if it exists
    if(flyweights.find(type) != flyweights.end()) {
        return flyweights[type];
    ...
    // Only handle WaterSupply for now
    if (type == "WaterSupply") {
        flyweights[type] = std::make_shared<WaterSupply>(
            "WaterSupply", 5000.0, 80, 15.0, true, 1, 30, 
            std::map<std::string, int>{{"Steel", 100}, {"Concrete", 200}});
    } else {
        throw std::out_of_range("Only WaterSupply is implemented for testing");
    }
